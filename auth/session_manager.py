...
class SessionManager:
    ...
    def save(self, cookies: list[dict]) -> None:
        ...
        logger.info("[SessionManager] Saved %d cookies → %s", len(validated), self._file)
        ...
        with self._lock:
            for path in (self._file, self._backup):
                if os.path.exists(path):
                    try: os.remove(path)
                    except OSError as e: logger.warning("Cannot remove %s: %s", path, e)
    ...
    def _backup_existing(self) -> None:
        if os.path.exists(self._file):
            try: shutil.copy2(self._file, self._backup)
            except OSError as e: logger.warning("Backup failed: %s", e)
